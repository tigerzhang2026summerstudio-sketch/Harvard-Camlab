...
export class Act2 {
  ...
  update(time, dt, ppwu) {
    ...
    if (s.phase === 'coda') {
      // THE DISSOLUTION — 一片，然后一起慢慢消失分散. No flicker, no
      // shards, no whiteout: the whole wall becomes ONE calm field of
      // light, held, then the standing sheet drifts slowly apart and
      // fades away together.
      //   form (0 – formEnd)     the light gathers into a full even field
      //   hold (formEnd–holdEnd) the field stands, complete and calm
      //   disperse (holdEnd – 1) replenishment stops; the sheet drifts
      //                          out on its own slow current and fades
      const cc = config.act2.coda;
      const ct = clamp01(s.phaseTime / config.acts.codaFadeSec);
      let field = 0;
      if (ct < cc.formEnd) field = ct / cc.formEnd;
      else if (ct < cc.holdEnd) field = 1;
      else field = 0;          // the standing field lives out its drift
      wind = 0;                // a calm dissolution — no turbulence
      this.post.setSwell(1);   // no whiteout flash

      this.floodAcc = (this.floodAcc ?? 0) + dt;
      while (this.floodAcc >= cc.fieldEverySec) {
        this.floodAcc -= cc.fieldEverySec;
        if (field <= 0.02) break;
        const n = Math.max(1, Math.round(cc.fieldBursts * field));
        for (let i = 0; i < n; i += 1) {
          const fx = rand(-0.5, 0.5);
          const fy = rand(-0.5, 0.5);
          // slow, soft, long-lived motes spread across the whole wall,
          // each drifting gently OUTWARD from center so the sheet opens
          // apart (分散) as it fades — never fast enough to sparkle.
          this.particles.burst({
            x: fx * config.worldWidth,
            y: fy * config.worldHeight,
            color: pick([config.palette.white, config.palette.gold,
              config.palette.white, config.palette.beryl]),
            count: cc.fieldCount,
            speed: 7, size: 2.8,
            life: rand(cc.fieldLifeMin, cc.fieldLifeMax),
            upBias: 0.12, jitter: 46,
            driftX: fx * cc.disperseSpeed,
            driftY: fy * cc.disperseSpeed * 0.7,
            minSpeedFrac: 0.15,
          });
        }
      }
